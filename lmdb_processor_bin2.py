...
MODEL_FILE = './predictor_data/model.b2.prototxt'
PRETRAINED = '/home/raul/Documents/git_repos_prv/data_tool/predictor_data/results2/krnet_bin/krnet_quick_iter_7000.caffemodel.h5'
...
def make_lmdb(total_images, data=[], output_file='out.lmdb', width=100, height=100, channels=3):
    ...
    env_train = lmdb.open('bin_train_lmdb', map_size=map_size)
    env_test = lmdb.open('bin_test_lmdb', map_size=map_size)
    cnt_train = 0
    cnt_test = 0
    cnt = 0
...
class DataBuilder:
    ...
    def __init__(self, days=78):
        self.session = Session()
        self.start_date = datetime.datetime.now() - datetime.timedelta(days=days)
