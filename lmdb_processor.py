...
MODEL_FILE = './predictor_data/model.full.prototxt'
...
def make_lmdb(total_images, data=[], output_file='out.lmdb', width=100, height=100, channels=3):
    ...
    env_train = lmdb.open('normal_train_lmdb', map_size=map_size)
    env_test = lmdb.open('normal_test_lmdb', map_size=map_size)
    cnt_train = 0
    cnt_test = 0
    cnt = 0
...
if __name__ == "__main__":
    if "build" in sys.argv:
        ...
        start_date = datetime.datetime(year=2017, month=1, day=16, hour=0, minute=0, second=0)
        end_date = datetime.datetime(year=2017, month=1, day=20, hour=23, minute=59, second=0)
