...
MODEL_FILE = './predictor_data/model.sigmoid.prototxt'
#PRETRAINED = './predictor_data/caffemodel.h5'
PRETRAINED = '/home/raul/Documents/git_repos_prv/data_tool/predictor_data/results2/krnet_sigmoid/krnet_full_sigmoid_iter_60000.caffemodel'
...
def make_lmdb(total_images, data=[], output_file='out.lmdb', width=100, height=100, channels=3):
    ...
    env_train = lmdb.open('full_train_lmdb', map_size=map_size)
    env_test = lmdb.open('full_test_lmdb', map_size=map_size)
    cnt_train = 0
    cnt_test = 0
    cnt = 0
...
class DataBuilder:
    ...
    def calc_label(self, tick_date, tick_close, minutes=60, profit=20.0):
        try:
            ...
            if float(tick_close) + profit*self.pip <= float(rmax) and float(rmin) >= float(tick_close) - profit*self.pip:
                return LABELS_MAP_R['buy15p']
            if float(tick_close) + profit*self.pip >= rmax and rmin <= float(tick_close) - profit*self.pip:
                return LABELS_MAP_R['sell15p']
            else:
                return LABELS_MAP_R['buysel15p']
        except Exception as ex:
...
class Predictor:
    ...
    def search_pattern(self, start_date, end_date, buy_threshold=0.2368, sell_threshold=0.3719):
        ...
        while True:
            ...
            #max_b.append((cpred[0]-cpred[1], cdate, clabel))
            #max_s.append((cpred[1]-cpred[0], cdate, clabel))
            max_b.append((cpred[0], cdate, clabel))
            max_s.append((cpred[1], cdate, clabel))
